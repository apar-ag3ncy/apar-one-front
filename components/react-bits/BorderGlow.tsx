 ...
 *
 * On the APAR featured-client cards it's tinted per brand: `glowColor` is each
 * card's accent, so the hover light matches the colour of the card.
 */
...
export interface BorderGlowProps {
  ...
  animated?: boolean;
  /** Array of (up to) 3 hex colors for the mesh-gradient border. */
  colors?: string[];
  /** Opacity multiplier for the near-edge mesh fill. */
  fillOpacity?: number;
}
...
function buildGlowVars(glowColor: string, intensity: number): CSSVars {
...
function buildGradientVars(colors: string[]): CSSVars {
  const vars: CSSVars = {};
  for (let i = 0; i < 7; i++) {
...
export default function BorderGlow({
  ...
  // NOTE: keep every custom-property value a STRING. React serializes numeric
  // custom props as strings on the server but as numbers on the client, which
  // trips a hydration mismatch warning. CSS `calc()` reads them fine either way.
  const style: CSSVars = {
