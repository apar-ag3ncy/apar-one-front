 ...
 *
 * One deliberate deviation from the stock shader: the final colour is remapped
 * to APAR's GOLD↔ORANGE palette (deep orange → warm gold → hot core) instead of
 * the stock rainbow, so it sits in the brand world. Geometry, glow, animation,
 * noise and transparency are all faithful to the original.
 */
  ...
  useEffect(() => {
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        // --- APAR gold↔orange palette (replaces the stock rainbow rgb) ---
        float inten = clamp(max(o.r, max(o.g, o.b)), 0.0, 1.0);
        vec3 cOrange = vec3(0.93, 0.34, 0.06);  // deep orange
        vec3 cGold   = vec3(1.00, 0.78, 0.36);  // warm gold
        vec3 cHot     = vec3(1.00, 0.96, 0.86); // hot core
        vec3 col = mix(cOrange, cGold, smoothstep(0.0, 0.55, inten));
        col = mix(col, cHot, smoothstep(0.78, 1.0, inten));
        col *= inten;
