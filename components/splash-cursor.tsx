 ...
 * devices, where a cursor trail makes no sense and the GPU cost isn't worth it.
 */
...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(c: HTMLCanvasElement) {
      ...
      let glCtx = c.getContext("webgl2", params) as any;
      const isWebGL2 = !!glCtx;
      if (!isWebGL2)
        glCtx = (c.getContext("webgl", params) || c.getContext("experimental-webgl", params)) as any;
    ...
    function getSupportedFormat(glCtx: any, internalFormat: number, format: number, type: number): any {
    ...
    function scaleByPixelRatio(input: number) {
      // cap DPR: the fluid is soft/blurry, so rendering past 1.5x is wasted GPU
      return Math.floor(input * Math.min(window.devicePixelRatio || 1, 1.5));
    }

    // cache the scaled canvas size so the hot loop never reads layout
    let cachedWidth = scaleByPixelRatio(canvas.clientWidth);
    let cachedHeight = scaleByPixelRatio(canvas.clientHeight);
    ...
    function updateFrame() {
      ...
      step(dt);
      render(null);
      if (Date.now() - lastActivity > 1600) {
        // dye has fully dissipated (DENSITY_DISSIPATION 3.5 clears in ~1.5s);
  ...
  return (
    <div className="splash-cursor" aria-hidden>
