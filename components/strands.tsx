 ...
 * Used as the stats-band backdrop so it reads distinctly from the Algolia-style
 * PixelDome beam used by the brand band / hero. Styles live in globals.css
 * (`.strands-container`), so there is no separate CSS import.
 */
...
void main() {
  ...
  // Envelope over the canvas WIDTH (not the height-normalized uv.x): upstream's
  // cos(uv.x * PI * 1.3) repeats its lobe on wide/short bands, scattering extra
  // strand sets at the sides. Width-normalized, exactly ONE centred set spans
  // the band and tapers off before the edges, at any aspect ratio.
  float nx = (gl_FragCoord.x / uResolution.x - 0.5) * 1.15;
  float env = abs(nx) < 0.5 ? pow(cos(nx * PI), uTaper) : 0.0;
...
export default function Strands({
  ...
  const propsRef = useRef<Required<Omit<StrandsProps, "className" | "style">>>(
    {} as Required<Omit<StrandsProps, "className" | "style">>
  );
  propsRef.current = {
    colors,
    count,
  ...
  useEffect(() => {
    ...
    let animateId = 0;
    let visible = true; // pause the loop entirely while the band is offscreen
    let palette = buildPalette(propsRef.current.colors);
    let colorsKey = propsRef.current.colors.join();
    let lastProps: typeof propsRef.current | null = null;
    ...
    const io = new IntersectionObserver(([en]) => {
      const nowVisible = en?.isIntersecting ?? true;
